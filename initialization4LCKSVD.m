...
% 504*1216
numClass = size(H_train,1); % number of objects  38*1216
numPerClass = round(dictsize/numClass); % initial points from each classes  500/38=13.13  13  每一类的原子数目
Dinit = []; % for LC-Ksvd1 and LC-Ksvd2
dictLabel = [];
for classid=1:numClass    % 38 类
...
for frameid=1:size(training_feats,2) %1216
    label_training = H_train(:,frameid);  % 某一列
    [maxv1,maxid1] = max(label_training);
    for itemid=1:size(Dinit,2)  %494
